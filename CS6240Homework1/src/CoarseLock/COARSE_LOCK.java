/*
 *	COARSE_LOCK: Main class and includes following methods:
 * 
...
class Runner1 implements Runnable {
	...
	// Constructor
	public Runner1(List<String> list, HashMap<String, int[]> accumulationDS, Boolean fibonaciiFlag) {
...
public class COARSE_LOCK {
	...
	public static void main(String args[]) {
		...
		String outputFilePath = "Output_COARSE_LOCK.txt";
		...
		COARSE_LOCK coarseLockObj = new COARSE_LOCK();
			...
			for (int i = 0; i < numberOfExecutions; i++) {
				...
				try {
					...
					for (int j = 0; j < processors; j++) {
						...
						int[] indices = coarseLockObj.getIndicesForSubList(list, j, processors);

						// create and start thread
						threadName[j] = new Thread(
								new CoarseLockRunner((list.subList(indices[0], indices[1])), accumulationDS, fibonaciiFlag));
						threadName[j].start();
					}
					...
					coarseLockObj.getAverageTemp(accumulationDS);
