/*
 *	 FINE_LOCK: Main class and includes following methods:
 * 
...
public class FINE_LOCK {
	...
	public static void main(String args[]) {
		...
		String filePath;
		List<String> list;
		ConcurrentHashMap<String, int[]> accumulationDS = new ConcurrentHashMap<String, int[]>();
		PrintWriter writer = null;
		String outputFilePath = "Output_FINE_LOCK.txt";
		...
		FINE_LOCK fineLockObj = new FINE_LOCK();
		...
		try {
			...
			for (int i = 0; i < numberOfExecutions; i++) {
				...
				// Clear the data structure for new execution
				accumulationDS.clear();
				...
				try {
					...
					for (int j = 0; j < processors; j++) {
						...
						int[] indices = fineLockObj.getIndicesForSubList(list, j, processors);

						// create and start thread
						threadName[j] = new Thread(
								new FineLockRunner((list.subList(indices[0], indices[1])), accumulationDS, fibonaciiFlag));
						threadName[j].start();
					}
					...
					fineLockObj.getAverageTemp(accumulationDS);
	...
	public HashMap<String, Integer> getAverageTemp(ConcurrentHashMap<String, int[]> accumulationDS) {
